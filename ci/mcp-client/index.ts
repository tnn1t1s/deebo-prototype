...
class MinimalMCPClient {
  ...
  private connected: boolean = false; // Track connection status internally

  constructor() {
    this.mcp = new Client({ name: "deebo-ci-client", version: "1.0.0" });
  }

  async connectToServer(deeboServerScriptPath: string) {
    if (this.connected) {
  ...
  async forceStartSession(args: Record<string, any>): Promise<string> {
    ...
    const result = await this.mcp.callTool({ name: "start", arguments: args }) as any; // Use 'as any' for now to bypass strict type checking on result
  ...
  async forceCheckSession(sessionId: string, maxRetries = 10): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      let text = "";
      try {
        result = await this.mcp.callTool({ name: "check", arguments: { sessionId } }) as any; // Use 'as any'
        // Access content directly from the assumed 'result' payload
